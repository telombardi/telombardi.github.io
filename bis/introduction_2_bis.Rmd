...
Even this simple form defines a process for collecting data so that it can be transformed into information. The CAPTCHA (__C__ompletely __A__utomated __P__ublic __T__uring test to tell __C__omputers and __H__umans __A__part) ensures that the user filling out the form is in fact a person rather than a bot. In other words, the website is designed to check the quality of the data being input to ensure that each record represents a real user. Quality data is much easier to transform into useful information.
...
[Classroom exercise: Find an example of career in technology that you find compelling.]
